...
void AI::YAAnnMdlStorage::saveMdlToDot(const YAAnn& yaann, const char* fileName)
{
    /*
    char labelName = 'u';
...
void AI::YAAnnMdlStorage::saveMdlToFile(const YAAnn& yaann, const char* fileName)
{
    /*
    std::ofstream ofs(fileName);
    if (ofs.is_open()) {
...
void AI::YAAnnMdlStorage::readMdlFromFile(YAAnn& yaann, const char* fileName)
{
    /*
    ...
    if (ifs.is_open()) {
        ...
        yaann.setModel(inputLayerSize, hiddenLayersNumber, hiddenLayerSize, outputLayerSize, false, 0);
        AI::YAAnnLayer* currLayer = yaann.getInputLayer();
        while (currLayer != yaann.getOutputLayer()) {
            for (int i = 0; i < currLayer->neuronsNumber(); ++i) {
                bool isBiased;
